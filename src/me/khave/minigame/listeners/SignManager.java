...
public class SignManager implements Listener {
    ...
    public void onSignChange(SignChangeEvent e) {
        if (e.getLine(0).equalsIgnoreCase("[MagicBattle]")) {
            ...
            catch (Exception ex) { MessageManager.getInstance().msg(e.getPlayer(), MessageManager.MessageType.BAD, "That is not a valid arena number!"); return; }

            Arena a = ArenaManager.getInstance().getArena(id);
            if (a == null) {
                MessageManager.getInstance().msg(e.getPlayer(), MessageManager.MessageType.BAD, "That is not a valid arena!");
                return;
            }
