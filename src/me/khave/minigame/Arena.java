...
//        this.signs = SignManager.getSigns(this);
    ...
    public void addPlayer(Player p) {
        if (currentPlayers >= numPlayers) {
            MessageManager.getInstance().msg(p, MessageManager.MessageType.BAD, "There are too many players.");
            return;
        }

        if (spawnPoint == null) {
            MessageManager.getInstance().msg(p, MessageManager.MessageType.BAD, "Spawn point not yet set.");
            return;
        }
...
/*        for (Sign s : signs) {
    ...
    public void stop(Player winner) {
        if (winner != null) MessageManager.getInstance().broadcast(MessageManager.MessageType.GOOD, winner.getName() + " has won arena " + id + "!");
    ...
    public void stop() {
        MessageManager.getInstance().broadcast(MessageManager.MessageType.GOOD, "The arena " + id + " has ended!");
    ...
    protected void sendMessage(MessageManager.MessageType type, String... messages) {
    ...
    private class Countdown extends BukkitRunnable {
        ...
        public void run() {
            if (timer == 0) {
                ...
                MessageManager.getInstance().broadcast(MessageManager.MessageType.GOOD, "The game has started!");
                a.state = ArenaState.STARTED;
                cancel();
            }

            if (countingNums.contains(timer)) {
                MessageManager.getInstance().broadcast(MessageManager.MessageType.INFO, msg.replace("%t", timer + ""));
            }
...
class PlayerData {
    ...
    protected Player getPlayer() {
        return Bukkit.getServer().getPlayer(puu.getUniqueId().toString());
    }

    protected void restorePlayer() {
        Player p = Bukkit.getServer().getPlayer(puu.getUniqueId().toString());
